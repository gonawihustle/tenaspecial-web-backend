...
# 2. Ensure SDK loaded in head
if 'supabase-js' not in content and '</head>' in content:
    content = content.replace('</head>', '  <script src="https://cdn.jsdelivr.net/npm/@supabase/supabase-js@2"></script>\n</head>')

# 3. Inject Channel UI & Direct Supabase Script
new_script = '''
<!-- SUPABASE CLOUD ENGINE FOR CHANNELS -->
<script>
(function() {
    const SUPABASE_URL = "https://xbuhtrxhzpjiiiqeluhh.supabase.co";
    const SERVICE_KEY = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InhidWh0cnhoenBqaWlpcWVsdWhoIiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImlhdCI6MTc4ODg4MTg4OCwiZXhwIjoyMTA0NDU3ODg4fQ.M2SWpbd0fiuSHkqXXjDRIfcupXQksWI3v0cHgo82F9o";
            ...
            if (channelHeader && channelHeader.parentElement) {
                ...
                if (!data || data.length === 0) {
                    ...
                    container.innerHTML = data.map(c => {
                        ...
                        const isPaid = c.is_paid || c.price > 0 || titleLower.includes('paid') || titleLower.includes('premium') || subLower.includes('paid') || subLower.includes('premium');
...
if '</body>' in content:
    ...
    with open('index.html', 'w', encoding='utf-8') as f:
        f.write(content)
    print("Channel UI and Cloud Sync script updated successfully!")
