...
# 1. Strip internal script blocks to eliminate conflicting legacy code
clean_html = re.sub(r'<script(?![^>]*\bsrc=)[^>]*>[\s\S]*?</script>', '', html)

# 2. Master Engine Injection
master_script = r'''
...
(function() {
    ...
    async function pushToCloud(payload) {
        try {
            ...
            if (res.ok) {
                alert("☁️ Saved to Cloud! Syncing across all browsers.");
                await syncFromCloud();
                return true;
            } else {
    ...
    // Stop form page reloads globally
    document.addEventListener('submit', function(e) {
        e.preventDefault();
        e.stopPropagation();
    }, true);

    // Global Click Handler with Explicit Section Isolation
    document.addEventListener('click', async function(e) {
        ...
        if (btnText.includes('clear all') || btnText.includes('wipe')) {
            e.preventDefault(); e.stopPropagation();
            if (confirm("⚠️ Wipe all existing data from cloud?")) {
        ...
        for (const [catName, docs] of Object.entries(groups)) {
            const docsList = docs.map(doc => {
                ...
                <div style="display:flex; justify-content:space-between; align-items:center; padding: 12px 0; border-bottom: 1px solid #1e293b;">
                    ...
                    <a href="${BOT_URL}" target="_blank" style="background:#0284c7; color:#ffffff; padding:6px 16px; border-radius:8px; font-size:12px; font-weight:bold; text-decoration:none;">Book Specialist</a>
                </div>`;
            }).join('');
...
if '</body>' in clean_html:
    final_html = clean_html.replace('</body>', master_script + '\n</body>')
else:
    final_html = clean_html + master_script

with open('index.html', 'w', encoding='utf-8') as f:
    f.write(final_html)

print("Root ancestor fix successfully applied!")
