...
# Completely remove any old engines
content = re.sub(r'<script id="tenaspecial-core">[\s\S]*?</script>', '', content)

# Using raw string to prevent escape sequence warnings
script = r'''
<!-- TENASPECIAL ROOT CLOUD ENGINE -->
...
(function() {
    ...
    async function pushToCloud(payload) {
        try {
            ...
            if (res.ok) {
                alert("☁️ SAVED TO CLOUD! Available on all browsers.");
                syncFromCloud(); // Refresh UI
                return true;
            } else {
                const errText = await res.text();
                alert("❌ CLOUD REJECTED IT: " + errText); // Tells us exactly if there is a permission issue
            }
        } catch(e) { alert("❌ NETWORK ERROR: Could not reach the cloud."); }
        return false;
    }

    // 2. KILL PAGE RELOADS GLOBALLY
    window.addEventListener('submit', (e) => {
        e.preventDefault();
        e.stopPropagation();
    }, true);

    // 3. CAPTURE-PHASE CLICK INTERCEPTOR (Runs BEFORE the browser can react)
    window.addEventListener('click', async (e) => {
        ...
        if (btnText.includes('save') || btnText.includes('add')) {
            // Find the container the button is in
            const container = btn.closest('form, div.p-4, div.card, div.flex-col, div.bg-gray-800, section, div') || document;
            const inputs = Array.from(container.querySelectorAll('input, select, textarea'));
            
            // Read placeholders to know WHAT we are saving, no matter what the button says
            const placeholders = inputs.map(i => (i.placeholder || '').toLowerCase()).join(' ');
            
            const isSpecialist = placeholders.includes('doctor') || placeholders.includes('speciality');
            const isProduct = placeholders.includes('product') || placeholders.includes('price');
            const isChannel = placeholders.includes('channel') || placeholders.includes('group') || (!isSpecialist && !isProduct && placeholders.includes('name'));

            if (isSpecialist || isProduct || isChannel) {
                e.preventDefault();
                e.stopPropagation(); // STOP THE BROWSER FROM REFRESHING

                if (isSpecialist) {
                    const docName = inputs.find(i => (i.placeholder||'').toLowerCase().includes('doctor name'))?.value;
                    const spec = inputs.find(i => (i.placeholder||'').toLowerCase().includes('speciality'))?.value || 'Specialist';
                    const customCat = inputs.find(i => (i.placeholder||'').toLowerCase().includes('category name'))?.value;
        ...
        else if (btnText.includes('clear all') && confirm("DANGER: Wipe entire cloud?")) {
            e.preventDefault(); e.stopPropagation();
            await fetch(SUPA_URL + '?title=not.is.null', { method: 'DELETE', headers: HEADERS });
            alert("☁️ Cloud wiped."); syncFromCloud();
        }
    }, true); // The "true" here is the magic that intercepts the click before anything else
    ...
    function getContainer(headerText, countLabel) {
        ...
        if (parentRow && parentRow.parentElement) {
            Array.from(parentRow.parentElement.children).forEach(child => {
                if (child !== parentRow && child !== container && child.tagName === 'DIV' && (child.textContent.includes('No ') || child.querySelector('button'))) {
    ...
    function updateSpecialistsUI(items) {
        ...
        if (!container) return;
        if (items.length === 0) return container.innerHTML = `<div style="padding:16px; background:#0f172a; border:1px dashed #1e293b; border-radius:12px; color:#64748b; font-size:12px; text-align:center;">No cloud categories.</div>`;
        ...
        for (const [catName, doctors] of Object.entries(grouped)) {
            ...
            <details style="background: #0f172a; border: 1px solid #1e293b; border-radius: 12px; margin-bottom: 12px; overflow:hidden;">
                <summary style="padding: 16px; cursor: pointer; font-weight: bold; color: white; display:flex; align-items:center; gap:12px; outline:none;">
                    ...
                    <span style="background: #1e293b; color:#cbd5e1; padding: 2px 8px; border-radius: 12px; font-size: 11px;">${doctors.length}</span>
                </summary>
    ...
    function updateProductsUI(items) {
        ...
        if (!container) return;
        if (items.length === 0) return container.innerHTML = `<div style="padding:16px; background:#0f172a; border:1px dashed #1e293b; border-radius:12px; color:#64748b; font-size:12px; text-align:center;">No cloud products.</div>`;
        ...
        container.innerHTML = items.map(item => {
            ...
            <div style="display:flex; justify-content:space-between; align-items:center; padding: 16px; background: #0f172a; border: 1px solid #1e293b; border-radius: 12px; margin-bottom: 12px;">
                ...
                <a href="${BOT_URL}" target="_blank" style="background:#9333ea; color:white; padding:10px 16px; border-radius:8px; font-weight:bold; font-size:13px; text-decoration:none;">Buy</a>
            </div>`;
        }).join('');
    }
    ...
    function updateChannelsUI(items) {
        ...
        if (!container) return;
        if (items.length === 0) return container.innerHTML = `<div style="padding:16px; background:#0f172a; border:1px dashed #1e293b; border-radius:12px; color:#64748b; font-size:12px; text-align:center;">No cloud channels.</div>`;
        ...
        container.innerHTML = items.map(item => {
            ...
            const btn = isPremium
                ...
                : `<a href="${item.link || BOT_URL}" target="_blank" style="display: flex; justify-content: center; align-items: center; width: 100%; padding: 10px; background: #1e293b; color: #22d3ee; font-size: 13px; font-weight: bold; border-radius: 8px; text-decoration: none; border: 1px solid #334155; margin-top: 12px;">Join Free ${kind} →</a>`;

            return `
                <div style="padding: 16px; margin-bottom: 12px; background: #0f172a; border: 1px solid #1e293b; border-radius: 12px;">
                    <div style="display: flex; align-items: center; gap: 8px;">
                        <span style="background: #334155; color: #cbd5e1; font-size: 10px; font-weight: bold; padding: 2px 6px; border-radius: 4px;">${kind.toUpperCase()}</span>
...
if '</body>' in content:
    ...
    with open('index.html', 'w', encoding='utf-8') as f:
        f.write(content)
    print("Root Cloud Interceptor deployed successfully!")
